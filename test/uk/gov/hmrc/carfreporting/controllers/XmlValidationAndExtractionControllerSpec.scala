  ...
  "XmlValidationAndExtractionController" - {
    "processXml" - {
      "must return OK (200) when the XML parser is successful" in {
        ...
        when(mockXmlParserService.validateAndExtractCARF(ArgumentMatchers.eq(path))).thenReturn(ResultT.fromValue(extractedFileDetailsCarf))
        ...
        verify(mockXmlParserService).validateAndExtractCARF(ArgumentMatchers.eq(path))
      }
      ...
      "must return Unprocessable Entity (422) when the XML parser fails with an XML error" in {
        ...
        when(mockXmlParserService.validateAndExtractCARF(ArgumentMatchers.eq(invalidPath)))
      ...
      "must return Internal Server Error (500) when the XML parser fails for unknown reasons" in {
        ...
        when(mockXmlParserService.validateAndExtractCARF(ArgumentMatchers.eq(path)))
    ...
    "processAEOIXml" - {
      "must return OK (200) when the XML parser is successful" in {
        ...
        when(mockXmlParserService.validateAndExtractAEOI(ArgumentMatchers.eq(path)))
        ...
        verify(mockXmlParserService).validateAndExtractAEOI(ArgumentMatchers.eq(path))
      }
      ...
      "must return Unprocessable Entity (422) when the XML parser fails with an XML error" in {
        ...
        when(mockXmlParserService.validateAndExtractAEOI(ArgumentMatchers.eq(invalidPath)))
          .thenReturn(ResultT.fromError(XmlErrors(Vector(validationError))))
