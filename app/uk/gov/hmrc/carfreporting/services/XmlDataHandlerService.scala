...
class XmlDataHandlerService @Inject() extends Logging {
  ...
  private def validateAndExtract[A](schema: XMLValidationSchema, inputStream: InputStream)(
      readXml: XMLStreamReader2 => A
  ) = {
      ...
      reader.next() match {
        case XMLStreamConstants.START_ELEMENT if !requestDetailCompleted =>
          ...
          localName match {
            ...
            case DETAILS
                if pathEndsWith(DETAILS, FILE_ERROR, VALIDATION_ERRORS, GENERIC_STATUS_MESSAGE, REQUEST_DETAIL) =>
            ...
            case DETAILS
                if pathEndsWith(DETAILS, RECORD_ERROR, VALIDATION_ERRORS, GENERIC_STATUS_MESSAGE, REQUEST_DETAIL) =>
              currentRecordErrorDetails = Some(readElement())
            case DOC_REF_ID_IN_ERROR
                if pathEndsWith(
                  DOC_REF_ID_IN_ERROR,
                  RECORD_ERROR,
                  VALIDATION_ERRORS,
